...
make                 # builds the two programs and all four tools
...
# or explicitly:
mpicxx -O2 -std=c++17 -o src/q1_mpi     src/q1_mpi.cpp
g++    -O2 -std=c++17 -o src/sequential src/sequential.cpp
```

| Component | Language | Purpose |
|---|---|---|
| `src/sequential`, `src/q1_mpi` | C++ | the programs |
| `tools/analyze` (`analyze.cpp`) | C++ | tables + SVG plots from `results.csv` |
| `tools/gen_matrix` (`gen_matrix.cpp`) | C++ | writes a problem file in the input format |
| `tests/run_tests.sh` | shell | runs the correctness suite |
| `benchmark/run_bench.sh` | shell | runs the timing sweep |
| `slurm/job_q1.sh` | shell | cluster submission |
...
output), and MPI vs the sequential reference at P = 1, 2, 4, 8. 25 cases:
...
plus three input-validation cases (truncated, overlong, missing header).
