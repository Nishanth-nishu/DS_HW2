...
cd src && make                        # builds both binaries
# or explicitly:
mpicxx -O2 -std=c++17 -o q6_mpi     q6_mpi.cpp
...
# oversubscribed / container environments:
...
Cases: PDF sample · single vertex · no edges · fully disconnected · single
...
```bash
python3 tools/gen_graph.py -v 100000 -e 1000000 -t random -s 103 -o big.in
python3 tools/gen_graph.py -v 100000 -t path -o path.in
python3 tools/gen_graph.py -v 100000 -t chain-blocks -c 16 -o chain.in
```

Topologies: `random`, `path`, `star`, `disconnected`, `isolated`, `cluster`,
`chain-blocks`. `--asymmetric` writes each edge into only one adjacency list.
...
│   └── run_tests.sh        sequential vs MPI at P=1,2,4,8
├── tools/
│   └── gen_graph.py        seeded generator, 7 topologies
├── benchmark/
│   ├── run_bench.sh        writes results.csv
│   ├── analyze.py          tables + plots
│   ├── results.csv         (produced by run_bench.sh)
│   └── plots/              (produced by analyze.py)
├── slurm/
