...
//   * phase breakdown    t_dist / t_compute / t_reduce
//
...
// Run  : ./tools/analyze [results.csv]      (from the Q7 project root)
// ---------------------------------------------------------------------------
...
struct Agg {
    double dist = 0, compute = 0, reduce = 0, total = 0, comm = 0, algo = 0;
};
...
int main(int argc, char** argv) {
    ...
    for (auto& kv : vComp) {
        ...
        a.reduce  = median(vGath[k]);
        a.total   = median(vTot[k]);
        a.comm    = a.dist + a.reduce;
    ...
    // ---- header ------------------------------------------------------------
    std::printf("# Q7 Benchmark Analysis - Server Log Analytics\n");
    ...
    std::printf("\n`t_algo` = `t_compute` + `t_comm`, where `t_comm` = "
                "`t_dist` (Bcast header + Scatterv records) + `t_reduce` "
                "(Reduce scalars + Gatherv per-key maps).\n");

    {
        std::vector<std::vector<std::string>> body;
        for (auto& s : sizes) body.push_back({s, shape[s]});
        harness::printTable("Input sizes", {"Label", "parameters"}, body);
    }
        ...
        for (auto& s : sizes)
            for (int p : procs) {
                ...
                body.push_back({s, std::to_string(p), fmt(a.dist, 4),
                                fmt(a.compute, 4), fmt(a.reduce, 4),
                                fmt(a.algo, 4)});
            }
        harness::printTable(
            "Phase breakdown (seconds, median)",
            {"Size", "P", "t_dist", "t_compute", "t_reduce", "t_algo"}, body);
    }
    ...
    harness::svgPlot("benchmark/plots/time.svg",
                     "Q7 - Execution time vs processes",
    ...
    harness::svgPlot(
        ...
        "Q7 - Communication share of algorithm time",
